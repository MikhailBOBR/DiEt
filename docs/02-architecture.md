...
## Фактическая схема компонентов

Backend является модульным монолитом. Поток запроса соответствует схеме:

```text
client → Express middleware/routes → module service → SQL adapter → PostgreSQL/SQLite
```

Это практическое разделение ответственности, а не строгая многослойная архитектура с независимыми `application`, `domain` и `data access` слоями.
...
Модули в [server/src/modules](../server/src/modules) группируют маршруты и операции отдельных поддоменов:
...
Сервисы содержат прикладные проверки, вычисления и SQL-запросы. Отдельный repository-слой в текущей реализации не вводился, поэтому сервисы не являются изолированным application layer.

### SQL adapter и инфраструктура БД

В [server/src/db](../server/src/db) находятся общий адаптер подключения, совместимость SQL для PostgreSQL/SQLite, инициализация схемы и seed-операции. Этот каталог не является самостоятельным data access layer: большая часть запросов располагается непосредственно в модульных сервисах. В Docker, CI и release-проверках база данных подключается как PostgreSQL backing service через `DB_PROVIDER=postgres` и `DB_*`/`DATABASE_URL`; SQLite остается только локальным fallback для быстрых изолированных тестов.
...
Текущая архитектура удовлетворяет требованиям fullstack CRUD-приложения и намеренно остается простой: `routes + service + SQL adapter`. При росте сложности SQL можно поэтапно выносить в repositories, но такой рефакторинг не требуется для корректного описания текущего решения.
